...
#define USBHIDASYNC_CHECK_DEVICE(device) \
...
static int poll_interrupt(int device) {
  ...
  libusb_fill_interrupt_transfer(transfer, usbdevices[device].devh, address, buf, size, (libusb_transfer_cb_fn)usb_callback, (void *)(unsigned long)device, 1000);
...
static void usb_callback(struct libusb_transfer* transfer)
{
  ...
  if(transfer->type == LIBUSB_TRANSFER_TYPE_INTERRUPT)
  {
    if(transfer->status == LIBUSB_TRANSFER_COMPLETED)
    {
      ...
      {
        usbdevices[device].callback.fp_read(usbdevices[device].callback.user, transfer->buffer, transfer->actual_length);
      }
    ...
    if(transfer->endpoint == usbdevices[device].config.endpoints.in.address && transfer->status != LIBUSB_TRANSFER_CANCELLED) {
...
int usbhidasync_write_timeout(int device, const void * buf, unsigned int count, unsigned int timeout)
{
  ...
  int ret = libusb_interrupt_transfer(usbdevices[device].devh, usbdevices[device].config.endpoints.out.address, (void *) buffer, usbdevices[device].config.endpoints.out.size - 1, &transferred, timeout * 1000);
  if(ret != LIBUSB_SUCCESS)
  {
...
int usbhidasync_read_timeout(int device, void * buf, unsigned int count, unsigned int timeout)
{
  ...
  int ret = libusb_interrupt_transfer(usbdevices[device].devh, usbdevices[device].config.endpoints.in.address, (void *) buf, count, &transferred, timeout * 1000);
  if(ret != LIBUSB_SUCCESS)
  {
    PRINT_ERROR_LIBUSB("libusb_interrupt_transfer", ret)
    return -1;
...
int claim_device(int device, libusb_device * dev) {
    ...
    ret = libusb_set_interface_alt_setting(usbdevices[device].devh, usbdevices[device].config.interface.number, usbdevices[device].config.interface.alternateSetting);
    if(ret != LIBUSB_SUCCESS)
    {
...
int usbhidasync_get_ids(int device, unsigned short * vendor, unsigned short * product) {
    ...
    USBHIDASYNC_CHECK_DEVICE(device)
...
int usbhidasync_write(int device, const void * buf, unsigned int count) {
  ...
  libusb_fill_interrupt_transfer(transfer, usbdevices[device].devh, usbdevices[device].config.endpoints.out.address, buffer, usbdevices[device].config.endpoints.out.size - 1, (libusb_transfer_cb_fn)usb_callback, (void *)(unsigned long)device, 1000);
